...
In this graph, it shows the death ratio, death / confirmed cases. Interestingly, the highest ratios are in Michigan, Florida, and West Virginia.
...
usa_row <- data.frame(
  ...
  cases  = sum(combined_by_state$cases),
  deaths = sum(combined_by_state$deaths)
) %>%
  mutate(ratio = deaths / cases * 100)

# combined_gbl <- bind_rows(combined_gbl, usa_row)
```
...
Checking the deaths divided by cases ratio, we see developed countries, like US, Europe have low death ratios, while developing countries have high death ratios.
